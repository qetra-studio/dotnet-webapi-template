...
{
	public Task VerifyTokenAsync(string token, Func<JwtSecurityToken, CancellationToken, Task> verify,
	...
	public Task VerifyTokenAsync<T>(string token, Func<JwtSecurityToken, CancellationToken, Task<T>> verify,
								 CancellationToken cancellationToken = default)
		=> logger.TimeAsync(() =>
		{
			var handler = new JwtSecurityTokenHandler();
	...
	public Task VerifySignatureAsync(string token, Action<TokenValidationParameters>? configure = null,
										   CancellationToken cancellationToken = default)
